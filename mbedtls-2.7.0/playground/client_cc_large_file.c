...
int main( int argc, char** argv )
{
    ...
    fseek(f, 0, SEEK_SET);
    // allocate memory
    mbedtls_printf("Allocating %d bytes of memory...\n", file_len + 1);
    ...
    len = bytes_read;
    mbedtls_printf("Number of bytes to write is %d\n", len);

    int total_written = 0;
    while( total_written != file_len) {
        ret = mbedtls_ssl_write(&ssl, buf + total_written, len - total_written);
        if (ret > 0) {
            total_written += ret;
            mbedtls_printf("Wrote %d bytes to server\n", ret);
    ...
    do
    {
        ...
        mbedtls_printf("Gonna try to read %d bytes\n", bytes_read - total_read);
        ...
        if ((bytes_read - total_read) == 0) {
            ret = 0;
            mbedtls_printf("Done reading. Closing the connection...\n");
            break;
        }
    }
    while( 1 );
